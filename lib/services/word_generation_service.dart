...
class WordGenerationService {
  ...
  Future<Either<Exception, String>> _getWordLema(WordGenerationInput word) async {
    final String prompt = """
...
""";
  ...
  Future<Either<Exception, Word>> getWord(WordGenerationInput word) async {
    return TaskEither<Exception, Word>(() async {
      ...
      // Generate word prompt and call Gemini
      final wordPrompt = _getWordPrompt(wordLema);
      final geminiPromptEither = await _geminiPromptService.generatePrompt(wordPrompt);
      ...
      try {
        ...
        throw Exception('Error decoding JSON: $error. Raw output: $cleanedOutput');
      }
